...
void ImGuiDrawer::SetupFonts() {
  ...
  // loaded before them - fonts cached here would dangle.
  {
    ...
    // NOTE(port): ImFontConfig::RasterizerGamma was removed in the imgui font
    // system rework (this tree pins imgui's docking branch). Left out to build;
    // restore the per-polarity gamma tuning if imgui is pinned to a version
    ...
    // config_on_light.RasterizerGamma = 1.0f;  // removed in imgui (see NOTE above)
